...
public class VeggiewayConfiguredFeatures {
    ...
    public static final ConfiguredFeature<?, ?> VEGGIEWAY_WILD_CORN_CONFIG = Feature.RANDOM_PATCH.withConfiguration((
                    new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(ModBlocks.VEGGIEWAY_WILD_CORN.get().getDefaultState()),
                            SimpleBlockPlacer.PLACER)).tries(CORN_PATCH_SIZE_VEGGIEWAY).zSpread(2).xSpread(2).build())
    ...
    public static final ConfiguredFeature<?, ?> VEGGIEWAY_WILD_LENTIL_CONFIG = Feature.RANDOM_PATCH.withConfiguration((
                    new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(ModBlocks.VEGGIEWAY_WILD_LENTIL.get().getDefaultState()),
                            SimpleBlockPlacer.PLACER)).tries(LENTIL_PATCH_SIZE_VEGGIEWAY).zSpread(2).xSpread(2).build())
            .withPlacement(Features.Placements.HEIGHTMAP_SPREAD_DOUBLE_PLACEMENT)
            .withPlacement(Features.Placements.VEGETATION_PLACEMENT).count(LENTIL_FREQUENCY_VEGGIEWAY);

    public static final ConfiguredFeature<?, ?> VEGGIEWAY_WILD_QUINOA_CONFIG = Feature.RANDOM_PATCH.withConfiguration((
                    new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(ModBlocks.VEGGIEWAY_WILD_QUINOA.get().getDefaultState()),
                            SimpleBlockPlacer.PLACER)).tries(QUINOA_PATCH_SIZE_VEGGIEWAY).zSpread(2).xSpread(2).build())
            .withPlacement(Features.Placements.HEIGHTMAP_SPREAD_DOUBLE_PLACEMENT)
            .withPlacement(Features.Placements.VEGETATION_PLACEMENT).count(QUINOA_FREQUENCY_VEGGIEWAY);

    public static final ConfiguredFeature<?, ?> VEGGIEWAY_WILD_SOYBEAN_CONFIG = Feature.RANDOM_PATCH.withConfiguration((
                    new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(ModBlocks.VEGGIEWAY_WILD_SOYBEAN.get().getDefaultState()),
                            SimpleBlockPlacer.PLACER)).tries(SOYBEAN_PATCH_SIZE_VEGGIEWAY).zSpread(2).xSpread(2).build())
            .withPlacement(Features.Placements.HEIGHTMAP_SPREAD_DOUBLE_PLACEMENT)
